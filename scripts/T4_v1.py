...
# -*- coding: utf-8 -*-
"""
Teste T4 - Compressão Gzip em texto com 20% de ruído artificial
...
ALGORITMO = "gzip_ruido"
VERSAO_SCRIPT = "v1"
ORIGEM = "local"
PERCENTAGEM_RUIDO = 0.20
COMENTARIO_PREVIO = f"Teste Gzip com {PERCENTAGEM_RUIDO*100:.0f}% de ruído artificial inserido"
...
def executar_teste():
    """
    Executa o teste T4 completo e retorna todas as métricas.
    
    Este teste introduz 20% de ruído artificial (caracteres aleatórios)
    no texto antes da compressão, permitindo observar como o aumento
    da entropia afeta a eficiência do algoritmo Gzip.
    """
    
    print(f"\n{'='*70}")
    print(f"Teste T4 - Gzip com {PERCENTAGEM_RUIDO*100:.0f}% Ruído")
    print(f"Modo: {MODO.upper()} | Início: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*70}\n")
    
    # Leitura do ficheiro original
    try:
    ...
    print(f"Ficheiro lido: {len(texto_original)} caracteres")
    
    # Introdução de ruído
    print(f"A introduzir {PERCENTAGEM_RUIDO*100:.0f}% de ruído...")
    texto_com_ruido = introduzir_ruido_texto(texto_original, PERCENTAGEM_RUIDO)
    
    dados_originais = texto_com_ruido.encode("utf-8")
    tamanho_original = len(dados_originais)
    
    print(f"Texto com ruído: {len(texto_com_ruido)} caracteres ({tamanho_original} bytes)")
    ...
    resultados = {
        "ficheiro": f"{FICHEIRO}_ruido_{int(PERCENTAGEM_RUIDO*100)}%",
        "algoritmo": ALGORITMO,
