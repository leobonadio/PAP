...
# -*- coding: utf-8 -*-
"""
Teste T5 - Compressão Gzip em ficheiro binário aleatório
...
# ===============================
FICHEIRO = "ficheiros_teste/binario_teste.bin"
TAMANHO_FICHEIRO_KB = 100
ALGORITMO = "gzip"
VERSAO_SCRIPT = "v1"
ORIGEM = "local"
COMENTARIO_PREVIO = f"Teste Gzip em ficheiro binário aleatório ({TAMANHO_FICHEIRO_KB} KB)"
...
def executar_teste():
    """
    Executa o teste T5 completo e retorna todas as métricas.
    
    Este teste gera um ficheiro binário com dados aleatórios e aplica
    compressão Gzip. Dados verdadeiramente aleatórios têm entropia máxima
    e redundância mínima, devendo resultar em baixa ou nula compressão.
    """
    
    print(f"\n{'='*70}")
    print(f"Teste T5 - Gzip em Ficheiro Binário")
    print(f"Modo: {MODO.upper()} | Início: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*70}\n")
    
    # Geração do ficheiro binário
    print(f"A gerar ficheiro binário aleatório ({TAMANHO_FICHEIRO_KB} KB)...")
    
    if not gerar_ficheiro_binario(FICHEIRO, TAMANHO_FICHEIRO_KB):
        print("ERRO ao gerar ficheiro binário.\n")
        return None
    
    print(f"Ficheiro '{FICHEIRO}' gerado com sucesso")
    
    # Leitura do ficheiro gerado
    try:
